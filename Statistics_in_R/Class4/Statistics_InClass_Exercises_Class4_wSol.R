...
qqPlot(water.qual$RESULT, group = water.qual$WATERSHED)
...
leveneTest(RESULT ~ WATERSHED, data = water.qual)
...
t.test.water.qual.95 <- t.test(RESULT ~ WATERSHED, data = water.qual, var.equal = FALSE)
t.test.water.qual.95
# Mean dissolved oxygen is significantly higher in Lady Bird Lake than Barton Creek

# Print confidence intervals
t.test.water.qual.95$conf.int
